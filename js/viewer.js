...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function updateImage(name) {
    ...
    const annoPath = 'images/' + encodeURIComponent(name) + '.anno';
    fetch(annoPath)
      ...
      .then(t => {
        ...
        annoElem.style.display = annoElem.textContent ? 'inline-block' : 'none';
      })
      .catch(_ => { annoElem.style.display = 'none'; });

    const idx = images.indexOf(name);
    progressElem.textContent = 'Image ' + (idx + 1) + ' of ' + images.length;
  }

  let currentIndex = images.indexOf(imgName);
  if (currentIndex === -1) { annoElem.textContent = 'Image not found.'; return; }
  ...
  arrowLeft.addEventListener('click', e => {
    ...
    window.location.search = '?img=' + encodeURIComponent(images[currentIndex]);
  });
